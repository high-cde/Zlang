...
pub enum Token { Let, Print, Hash, Broadcast, Exec, Identifier(String), Int(i64), Str(String), Plus, Minus, Eq, EOF }
...
impl Lexer {
    ...
    pub fn next_token(&mut self) -> Token {
        ...
        match ch {
            ...
            '=' => { self.pos += 1; Token::Eq }
            '"' => self.read_string(),
            _ => { if ch.is_alphabetic() { self.read_identifier() } else if ch.is_numeric() { self.read_number() } else { self.pos += 1; self.next_token() } }
        }
    }
    ...
    fn read_identifier(&mut self) -> Token {
        ...
        match s.as_str() { 
            ...
            "exec" => Token::Exec, // <-- IL PONTE AL SISTEMA
